...
TextPtrLo      word
               ...
               ora   #$0100             modifiy our ID passed in accumulator
               sta   myID
               ...
               tsc
               tcd
               lda   [TextPtrLo]        length byte
               beq   use_default

               shortm
               lda   [TextPtrLo]        get length byte (again!)
               sta   DirBuffer
               stz   DirBuffer+2

               ldy   #1                 make length byte into length word!
               ldx   #0
string         lda   [TextPtrLo],y
               sta   nom,x
               iny
               inx
               cpx   dirbuffer          copied whole string?
               bne   string
               longm
...
Open_Prms      anop
               dc    i'2'               pcount
ref_num        ds    2
Name_Ptr       dc    i4'DefaultDir'     pointer to directory
DefaultDir     GSStr '0:'

DirBuffer      anop
               dc    i'0000'            length word
nom            ds    33
