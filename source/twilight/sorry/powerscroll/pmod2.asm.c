...
noSlow         lda   TextPtr,s           >long,x addressing instead of [dp],y.
               inc   a                   [We're adding 1 to it to skip over the
               sta   FillStr1+1           length byte at the beginning of it]
               sta   FillStr2+1
               sta   FillStr3+1
               lda   TextPtr+2,s
               sta   FillStr1+3
...
wait           lda   >Gun               We wait for the electron gun to be
               cmp   #(topline18/2)+$80  refreshing the line below the last
               blt   wait                line of our scroll before we start
               longm                     redrawing, so the drawing won't tear.

               jsr   Scroll             Scroll over our text!
...
LetterAddrTab  anop
               dc    a'lspace,lexp,no,no,no,no,no,lapos,no,no,no,no'
